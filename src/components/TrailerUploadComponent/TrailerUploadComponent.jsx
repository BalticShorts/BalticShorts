...
export function TrailerUploadComponent(props) {
  ...
  const handleUpload = async () => {
    ...
    const location = `balticshortsphotos/trailer/${uId}`;
    if (uploadedFiles.length !== 0) {
      if (props.movie) {
        props.movie.trailer_location = location;
      }
      const trailerLoc = location + "/" + encodeURIComponent(uploadedFiles[0].name);
      props.videoLoc.push(trailerLoc);
    } else {
      setUploading(false);
      return;
    }
