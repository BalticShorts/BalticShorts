...
import { listTestPersonRoles } from "../graphql/queries";
import { createTestPerson, updateTestPersonRole } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function TestPersonCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const testPerson = (
            await API.graphql({
              query: createTestPerson.replaceAll("__typename", ""),
              variables: {
                input: {
                  ...modelFieldsToSave,
                },
              ...
              promises.push(
                API.graphql({
                  ...
                  variables: {
                    input: {
                      id: original.id,
                      testPersonRolesId: testPerson.id,
                    },
                  },
                })
              );
      ...
      {...getOverrideProps(overrides, "TestPersonCreateForm")}
      {...rest}
    >
      <TextField
        label="Name"
