...
import { updateMovieRole } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function MovieRoleUpdateForm(props) {
  const {
    id: idProp,
    movieRole: movieRoleModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = movieRoleRecord
      ? { ...initialValues, ...movieRoleRecord, Movie, Role, roleID }
      : initialValues;
  ...
  const [movieRoleRecord, setMovieRoleRecord] =
    React.useState(movieRoleModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getMovieRole.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getMovieRole
        : movieRoleModelProp;
      ...
      setMovieRoleRecord(record);
    };
    queryData();
  }, [idProp, movieRoleModelProp]);
  React.useEffect(resetStateValues, [movieRoleRecord, Movie, Role, roleID]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            ...
            roleMovieRoleId: modelFields?.Role?.id ?? null,
            roleID: modelFields.roleID,
          };
          await API.graphql({
            query: updateMovieRole.replaceAll("__typename", ""),
            variables: {
              input: {
                id: movieRoleRecord.id,
                ...modelFieldsToSave,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "MovieRoleUpdateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || movieRoleModelProp)}
