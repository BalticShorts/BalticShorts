...
import { updateTestPersonRole } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function TestPersonRoleUpdateForm(props) {
  const {
    id: idProp,
    testPersonRole: testPersonRoleModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = testPersonRoleRecord
      ? {
          ...initialValues,
          ...testPersonRoleRecord,
          person,
          role,
          testPersonRolesId,
        }
    ...
    setTestPersonRolesId(cleanValues.testPersonRolesId);
    setCurrentTestPersonRolesIdValue(undefined);
    setCurrentTestPersonRolesIdDisplayValue("");
    setErrors({});
  };
  const [testPersonRoleRecord, setTestPersonRoleRecord] = React.useState(
    testPersonRoleModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getTestPersonRole.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTestPersonRole
        : testPersonRoleModelProp;
      ...
      setRole(roleRecord);
      const testPersonRolesIdRecord = record
        ? record.testPersonRolesId
        : undefined;
      const testPersonRecord = testPersonRolesIdRecord
        ? (
            await API.graphql({
              query: getTestPerson.replaceAll("__typename", ""),
              variables: { id: testPersonRolesIdRecord },
            })
          )?.data?.getTestPerson
        : undefined;
      setTestPersonRolesId(testPersonRolesIdRecord);
      setSelectedTestPersonRolesIdRecords([testPersonRecord]);
      setTestPersonRoleRecord(record);
    };
    queryData();
  }, [idProp, testPersonRoleModelProp]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          person: person ?? null,
          role: role ?? null,
          testPersonRolesId: testPersonRolesId ?? null,
        };
        ...
        try {
          ...
          const modelFieldsToSave = {
            ...
            testPersonRolesId: modelFields.testPersonRolesId ?? null,
          };
          await API.graphql({
            query: updateTestPersonRole.replaceAll("__typename", ""),
            variables: {
              input: {
                id: testPersonRoleRecord.id,
                ...modelFieldsToSave,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "TestPersonRoleUpdateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
      ...
      >
        <Autocomplete
          ...
          defaultValue={testPersonRolesId}
          onChange={(e) => {
            let { value } = e.target;
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || testPersonRoleModelProp)}
