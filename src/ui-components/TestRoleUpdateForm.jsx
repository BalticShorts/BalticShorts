...
export default function TestRoleUpdateForm(props) {
  const {
    id: idProp,
    testRole: testRoleModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = testRoleRecord
      ? {
          ...initialValues,
          ...testRoleRecord,
          movieRoles: linkedMovieRoles,
          personRoles: linkedPersonRoles,
        }
      : initialValues;
    setName(cleanValues.name);
    setMovieRoles(cleanValues.movieRoles ?? []);
  ...
  const [testRoleRecord, setTestRoleRecord] = React.useState(testRoleModelProp);
  const [linkedMovieRoles, setLinkedMovieRoles] = React.useState([]);
  const canUnlinkMovieRoles = true;
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getTestRole.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTestRole
        : testRoleModelProp;
      const linkedMovieRoles = record?.movieRoles?.items ?? [];
      setLinkedMovieRoles(linkedMovieRoles);
      const linkedPersonRoles = record?.personRoles?.items ?? [];
      setLinkedPersonRoles(linkedPersonRoles);
      setTestRoleRecord(record);
    };
    queryData();
  }, [idProp, testRoleModelProp]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const promises = [];
          const movieRolesToLink = [];
          const movieRolesToUnLink = [];
          const movieRolesSet = new Set();
          const linkedMovieRolesSet = new Set();
          movieRoles.forEach((r) =>
            movieRolesSet.add(getIDValue.movieRoles?.(r))
          );
          linkedMovieRoles.forEach((r) =>
            linkedMovieRolesSet.add(getIDValue.movieRoles?.(r))
          );
          linkedMovieRoles.forEach((r) => {
            if (!movieRolesSet.has(getIDValue.movieRoles?.(r))) {
              movieRolesToUnLink.push(r);
            }
          });
          movieRoles.forEach((r) => {
            if (!linkedMovieRolesSet.has(getIDValue.movieRoles?.(r))) {
              movieRolesToLink.push(r);
            }
          });
          movieRolesToUnLink.forEach((original) => {
            if (!canUnlinkMovieRoles) {
              throw Error(
                `TestMovieRole ${original.id} cannot be unlinked from TestRole because undefined is a required field.`
              );
            }
            promises.push(
              API.graphql({
          ...
          movieRolesToLink.forEach((original) => {
            promises.push(
              API.graphql({
                query: updateTestMovieRole.replaceAll("__typename", ""),
                variables: {
          ...
          personRolesToUnLink.forEach((original) => {
            if (!canUnlinkPersonRoles) {
              throw Error(
                `TestPersonRole ${original.id} cannot be unlinked from TestRole because undefined is a required field.`
              );
            }
            promises.push(
              API.graphql({
          ...
          promises.push(
            API.graphql({
              query: updateTestRole.replaceAll("__typename", ""),
              variables: {
                input: {
                  id: testRoleRecord.id,
                  ...modelFieldsToSave,
                },
              },
            })
      ...
      {...getOverrideProps(overrides, "TestRoleUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || testRoleModelProp)}
