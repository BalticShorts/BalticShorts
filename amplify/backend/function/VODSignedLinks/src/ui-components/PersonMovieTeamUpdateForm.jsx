...
import { updatePersonMovieTeam } from "../src/graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function PersonMovieTeamUpdateForm(props) {
  const {
    id: idProp,
    personMovieTeam: personMovieTeamModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = personMovieTeamRecord
      ? { ...initialValues, ...personMovieTeamRecord, MovieTeam, Person, Role }
      : initialValues;
  ...
  const [personMovieTeamRecord, setPersonMovieTeamRecord] = React.useState(
    personMovieTeamModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getPersonMovieTeam.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPersonMovieTeam
        : personMovieTeamModelProp;
      ...
      setRole(RoleRecord);
      setPersonMovieTeamRecord(record);
    };
    queryData();
  }, [idProp, personMovieTeamModelProp]);
  React.useEffect(resetStateValues, [
    personMovieTeamRecord,
    MovieTeam,
    Person,
    Role,
  ]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            movieteamID: modelFields?.MovieTeam?.id ?? null,
      ...
      {...getOverrideProps(overrides, "PersonMovieTeamUpdateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || personMovieTeamModelProp)}
