...
exports.handler = async (event) => {
  try {
    ...
    if (!keyId) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "Missing keyId" })
      };
    }

    // Decode the communication key from base64
    let communicationKey = Buffer.from(communicationKeyAsBase64, "base64");

    // Define the keyId and messages
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ token: jwtAsString })
    };
  } catch (error) {
    console.error("Error generating JWT:", error);
    return {
      statusCode: 500,
      body: JSON.stringify({ error: "Failed to generate JWT" })
    };
  }
};
